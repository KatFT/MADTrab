...
var y{p in PROD,t in TRAB}>=0,<=d[p,t]; #vai ser usada para a restriçao da produçao
...
maximize revenue:
	sum {i in I} (sum { p in PROD} mars[i,p]*x[i,p] + sum { p in PROD} venus[i,p]*x[i,p] +  sum { p in PROD} mercury[i,p]*x[i,p] );
