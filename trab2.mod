...
Cap {i in B, p in PROD,t in TRAB} :sum {l in PLAT} produzido[l,p,i]<=producao[t,p];
#------------------------------------#
#ignorar isto->É só pegar no produced / dividir pela tabela e a soma disso para uma dada linha de produção tem que ser <= 1
solve;
printf "\n-----------------------------------------\n";
printf "\nExercício 4:\n";
printf "\nValor ótimo: %g\n\n", revenue;
